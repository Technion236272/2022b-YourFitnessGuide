...
class EditWorkout extends StatefulWidget {
  late var postData;
  EditWorkout({Key? key, this.postData}) : super(key: key);
...
class _EditWorkoutState extends State<EditWorkout> {
  ...
  Widget _buildExercises(double height, double width) {
    ...
    if (_textFieldInput.isEmpty) {
      for (int i = 0; i < postData['exercises']!.length; i++) {
        _addInputField2(context, postData['exercises']![i]);
      }
    }
      ...
      children: [
        ...
        Expanded(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              const Text("Exercises", style: TextStyle(color: appTheme, fontSize: 20)),
              ListView.builder(
                shrinkWrap: true,
  ...
  initState() {
    super.initState();
    workoutNameController.text = postData['title']!;
                  ...
                  children: [
                    _isLoading
                        ...
                        onPressed: () async {
                          ...
                          bool selectedGoal = loseWeight! || gainMuscle! || gainWeight! || maintainHealth!;
                          if (title == "" || description == "" || selectedGoal == false || _exercises.isEmpty) {
                            ...
                            if (isSubmitted) {
                              const snackBar = SnackBar(content: Text('Workout edited successfully'));
                              ScaffoldMessenger.of(context).showSnackBar(snackBar);
