...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: const Text('Reset your password'), backgroundColor: appTheme),
        body: SingleChildScrollView(
          child: Column(
            children: [
              SizedBox(height: resetHeight * 0.04),
              const Text(
                  'Enter the email you use to sign in below',
                  style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)
              ),
              SizedBox(height: resetHeight * 0.05),
              Container(
                padding: EdgeInsets.symmetric(
                    vertical: resetHeight * 0.01, horizontal: resetWidth * 0.1),
                child: _buildEmail(resetHeight),
              ),
              Container(child: _buildSubmitReset(resetHeight, resetWidth)),
              SizedBox(height: resetHeight * 0.39),
              Image.asset(
